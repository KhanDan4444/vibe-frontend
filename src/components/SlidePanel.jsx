  ...
  return createPortal(
    <div className={`fixed inset-0 ${zIndexClass} flex justify-end`}>
      ...
      >
        ...
        {footer && (
          <div className="safe-bottom shrink-0 border-t border-slate-200/80 bg-gradient-to-t from-slate-50 to-white px-4 py-3 shadow-[0_-8px_24px_-12px_rgba(15,23,42,0.12)] dark:border-app-border-subtle dark:from-app-raised dark:to-app-surface dark:shadow-[0_-8px_24px_-12px_rgba(0,0,0,0.45)] sm:px-6 sm:py-3.5">
            {footer}
          </div>
        )}
      </aside>
    ...
    return (
      ...
      >
        <span
          ...
          aria-hidden
        />
        {Icon && <Icon className="relative h-4 w-4 shrink-0" aria-hidden />}
  ...
  if (variant === 'tile') {
    return (
      <button
        type="button"
        onClick={onClick}
        className={`group flex min-h-[40px] cursor-pointer items-center gap-2 rounded-xl border border-slate-200/90 bg-white px-3 py-2 text-left transition hover:border-indigo-300 hover:bg-indigo-50/70 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 dark:border-app-border-subtle dark:bg-app-raised dark:hover:border-indigo-500/40 dark:hover:bg-indigo-500/10 disabled:opacity-50 ${className}`}
        {...props}
      >
        {Icon && (
          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-slate-600 transition group-hover:bg-indigo-100 group-hover:text-indigo-700 dark:bg-app-surface dark:text-app-muted dark:group-hover:bg-indigo-500/20 dark:group-hover:text-indigo-300">
  ...
  if (variant === 'successHero') {
    return (
      <button
        type="button"
        onClick={onClick}
        className={`group relative flex w-full min-h-[40px] cursor-pointer items-center justify-center gap-2 overflow-hidden rounded-xl bg-emerald-600 px-3 py-2.5 text-sm font-semibold text-white shadow-sm shadow-emerald-600/20 transition hover:bg-emerald-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-emerald-400 focus-visible:ring-offset-2 dark:shadow-emerald-900/40 dark:focus-visible:ring-offset-app-surface disabled:opacity-50 ${className}`}
        {...props}
      >
        {Icon && <Icon className="relative h-4 w-4 shrink-0" aria-hidden />}
        <span className="relative">{children}</span>
