  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-slate-50 p-6">
      <div className="w-full max-w-md rounded-2xl border border-rose-200 bg-white p-8 shadow-sm text-center">
        ...
        <h1 className="text-xl font-bold text-slate-900 dark:text-app-text-strong">{t('lockout.title')}</h1>
        {gymName && (
          <p className="mt-2 text-sm font-semibold text-slate-800">{gymName}</p>
        )}
        <p className="mt-2 text-sm text-slate-600 dark:text-app-text">{t('lockout.body')}</p>
        <button
          type="button"
          onClick={logout}
